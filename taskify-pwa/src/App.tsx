...
export default function App() {
  ...
  const [upcomingButtonPosition, setUpcomingButtonPosition] = useState<{
    ...
    const margin = 16;
    const buttonSize = 44;
    const defaultBottomGap = 80;
    const maxY = Math.max(margin, window.innerHeight - buttonSize - margin);
    const preferredY = window.innerHeight - buttonSize - defaultBottomGap;
    return { side: "right", y: Math.min(Math.max(margin, preferredY), maxY) };
  });
  ...
  const clampUpcomingButtonY = useCallback(
    (y: number) => {
      ...
      const margin = 16;
      const buttonHeight = upcomingButtonRef.current?.offsetHeight ?? 44;
  ...
  const handleUpcomingPointerDown = useCallback(
    (e: React.PointerEvent<HTMLButtonElement>) => {
      upcomingDragState.current = {
        ...
        originY: upcomingButtonPosition.y,
        startY: e.clientY,
      };
  ...
  const handleUpcomingPointerMove = useCallback(
    (e: React.PointerEvent<HTMLButtonElement>) => {
      ...
      e.preventDefault();
      const { originY, startY } = upcomingDragState.current;
      const deltaY = e.clientY - startY;
      if (!upcomingDragMoved.current && Math.abs(deltaY) > 1) {
  ...
  const commitUpcomingSide = useCallback((clientX: number | null) => {
    if (typeof window === "undefined" || clientX === null) return;
    setUpcomingButtonPosition((prev) => ({
      ...prev,
  ...
  const handleUpcomingPointerEnd = useCallback(
    (e: React.PointerEvent<HTMLButtonElement>) => {
      ...
      commitUpcomingSide(e.clientX ?? null);
    },
    [commitUpcomingSide]
  );
  ...
  return (
    <div className="min-h-screen px-4 py-4 sm:px-6 lg:px-8 text-primary">
      ...
      <button
        ...
        style={{
          top: `${upcomingButtonPosition.y}px`,
          ...(upcomingButtonPosition.side === "left" ? { left: "1rem" } : { right: "1rem" }),
        }}
