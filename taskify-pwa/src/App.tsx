        ...
        <header className="flex items-center gap-3 mb-4">
          ...
          <div className="ml-auto flex gap-2">
            ...
            <button
              ...
              onClick={() => {
                alert(
                  "To install: Share ▸ Add to Home Screen (iOS Safari), or use the install icon in Chrome"
                );
              }}
            >
              Install
...
function Column({
  ...
  return (
    <div className="rounded-2xl bg-neutral-900/60 border border-neutral-800 p-3 min-h-[18rem]" ref={ref}>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <button
          onClick={onComplete}
          className="opacity-0 group-hover:opacity-100 transition px-2 py-1 rounded bg-emerald-600"
        >
          ✓
        </button>
          ...
          {(Array.from({ length: 7 }, (_, i) => i as Weekday)).map((d) => {
            ...
            return (
              <button
                key={d}
                onClick={() => toggleDay(d)}
                className={`px-2 py-2 rounded ${on ? "bg-emerald-600" : "bg-neutral-800"}`}
              >
                {WD_SHORT[d]}
              </button>
            );
